...
class InputText:
    ...
    def __init__(self, width : int, height : int, x_cor : int, y_cor : int, base_text : str,name_image : str, screen_name : str, font_name : str):
        ...
        Атрибуты:
        - :mod:`width`: Ширина поля ввода.
        - :mod:`height`: Высота поля ввода.
        - :mod:`x_cor`: Координата X для позиционирования поля ввода на экране.
        - :mod:`y_cor`: Координата Y для позиционирования поля ввода на экране.
        - :mod:`base_text`: Базовый текст, который отображается в поле ввода до ввода пользователем.
        - :mod:`screen_name`: Экран (поверхность), на котором будет отображаться поле ввода.
        - :mod:`font_name`: Название шрифта, который будет использоваться для текста в поле.
        - :mod:`user_text`: Текст, введенный пользователем. Изначально равен `base_text`.
        - :mod:`active`: Флаг, который указывает, активно ли поле ввода.
        - :mod:`name_image`: Название изображения, которое будет использовано в качестве фона поля ввода.
        - :mod:`font`: Шрифт, который будет использоваться для текста.
        - :mod:`max_length`: Максимальная длина текста, который можно ввести в поле.
        - :mod:`rect`: Прямоугольник, определяющий область, в которой отображается поле ввода.
        - :mod:`VISIBLE`: Прозрачность (255 — полностью видимый, 0 — невидимый).

        Пример использования : 
        ```python
        input_password = InputText(width = 346 , height = 85 , x_cor = 470, y_cor= 195, font_name = "Jersey15.ttf" , screen_name = main_screen, 
        base_text= "password", name_image= "input_area.png")
        ```
        '''
    ...
    def load_font(self, font_name : str): 
        '''
        `Метод` загрузки шрифта, и `трансформации` с нужными нам `атрибуты`
        '''
    ...
    def check_event(self, event: object):
        '''
        `Метод` который проверяет что `сейчас` происходит
        '''
    ...
    def draw_text(self):
        '''
        `Метод` который отрисовывает поле ввода текста и его внешний вид
        '''
